// SPDX-FileCopyrightText: 2018 Contributors to Chatterino
...
Label *addCopyableLabel(LayoutCreator<QHBoxLayout> box, const char *tooltip,
  ...
  QObject::connect(
      button.getElement(), &Button::leftClicked, [label = label.getElement()] {
...
UserInfoPopup::UserInfoPopup(bool closeAutomatically, Split *split)
    : DraggablePopup(closeAutomatically, split), split_(split),
  ...
  HotkeyController::HotkeyMap actions{
         ...
         if (target == "ban") {
           ...
           if (static_cast<int>(timeoutButtons.size()) < buttonNum ||
             ...
             return QString("Invalid argument for execModeratorAction: "
                 ...
                 .arg(buttonNum, static_cast<int>(timeoutButtons.size()) - 1);
           }
      ...
      // these actions make no sense in the context of a usercard, so they
      // aren't implemented
      {"reject", nullptr},
      {"accept", nullptr},
      {"openTab", nullptr},
  ...
  auto head = layout.emplace<QHBoxLayout>().withoutMargin();
  {
    ...
    QObject::connect(
        avatar.getElement(), &Button::clicked, [this](Qt::MouseButton button) {
          if (this->isKick_) {
            this->onKickProfilePictureClick(button);
            return;
          }

          QUrl channelURL("https://www.twitch.tv/" + this->userName_.toLower());
          ...
          case Qt::RightButton: {
            // don't raise open context menu if there's no avatar (probably in
            // cases when invalid user's usercard was opened)
            ...
            menu->addAction("Copy avatar link",
                            [avatarUrl] { crossPlatformCopy(avatarUrl); });
            ...
            menu->addAction(
                "Open channel in a new popup window", this, [loginName] {
                  auto *app = getApp();
                  auto &window =
            ...
            menu->addAction("Open channel in a new tab", this, [loginName] {
              ChannelPtr channel =
                  getApp()->getTwitch()->getOrAddChannel(loginName);
    ...
    switchAv->hide();
    QObject::connect(switchAv.getElement(), &LabelButton::leftClicked, [this] {
    ...
    auto vbox = head.emplace<QVBoxLayout>();
    {
      // items on the right
      {
        auto box = vbox.emplace<QHBoxLayout>().withoutMargin().withoutSpacing();
        ...
        // addCopyableLabel adds the copy button last -> add the indicator
        // before that
        box->insertWidget(box->count() - 1, this->ui_.liveIndicator);
        box->insertItem(box->count() - 1, ScalingSpacerItem::horizontal(7));
        box->addSpacing(5);
        box->addStretch(1);

        this->ui_.localizedNameLabel = addCopyableLabel(
            box, "Copy localized name", &this->ui_.localizedNameCopyButton);
  ...
  auto user = layout.emplace<QHBoxLayout>().withoutMargin();
  {
    ...
    user.emplace<LabelButton>("Add notes", this).assign(&this->ui_.notesAdd);
    ...
    QObject::connect(userlogs.getElement(), &Button::leftClicked, [this] {
      QDesktopServices::openUrl(
          "https://tv.supa.sh/logs?c=" + this->underlyingChannel_->getName() +
          "&u=" + this->userName_);
    });
    ...
    // userstate
    // We can safely ignore this signal connection since this is a private
    // signal, and we only connect once
    std::ignore = this->userStateChanged_.connect(
        [this, mod, unmod, vip, unvip]() mutable {
          ...
          if (twitchChannel) {
            ...
            visibilityModButtons = twitchChannel->isBroadcaster() && !isMyself;
          }
  ...
  {
    auto timeout =
        moderation.emplace<TimeoutWidget>().assign(&this->ui_.timeoutWidget);
    ...
    std::ignore = timeout->buttonClicked.connect([this](auto item) {
      ...
      case TimeoutWidget::Timeout: {
        if (this->underlyingChannel_) {
          QString value =
              "/timeout " + this->userName_ + " " + QString::number(arg) + 's';
...
void UserInfoPopup::updateUserData() {
  ...
  const auto onUserFetched = [this, hack, currentUser](const HelixUser &user) {
    if (!hack.lock()) {
      return;
    }
    ...
    if (user.displayName.toLower() != user.login) {
      ...
      this->ui_.localizedNameLabel->setProperty("copy-text", user.displayName);
          ...
          [this, hack](const IvrSubage &subageInfo) {
            ...
            if (!subageInfo.followingSince.isEmpty()) {
              QDateTime followedAt =
                  QDateTime::fromString(subageInfo.followingSince, Qt::ISODate);
            ...
            if (subageInfo.isSubHidden) {
              this->ui_.subageLabel->setText("Subscription status hidden");
  ...
  const auto onUserFetchFailed = [this, hack] {
    ...
    this->ui_.followerCountLabel->setText(TEXT_FOLLOWERS.arg(TEXT_UNAVAILABLE));
  ...
  if (!this->userId_.isEmpty()) {
    getHelix()->getUserById(this->userId_, onUserFetched, onUserFetchFailed);
  } else {
...
void UserInfoPopup::updateKickUserData() {
  ...
  auto onChannelFetchFailed = [](UserInfoPopup *self) {
    ...
    self->ui_.followerCountLabel->setText(TEXT_FOLLOWERS.arg(TEXT_UNAVAILABLE));
  ...
  auto onChannelFetched = [](UserInfoPopup *self,
    ...
    self->setWindowTitle(TEXT_TITLE.arg(channel.user.username,
                                        self->underlyingChannel_->getName()));
  ...
  KickApi::privateChannelInfo(
      this->userName_, [self = QPointer(this), onChannelFetched,
        ...
        if (res) {
          ...
          qCDebug(chatterinoKick) << "Channel fetch failed" << res.error();
      ...
      [self = QPointer(this)](const auto &res) {
        ...
        if (res->followingSince) {
          ...
          self->ui_.followageLabel->setToolTip(
              formatLongFriendlyDuration(*res->followingSince,
                                         QDateTime::currentDateTimeUtc()) +
              u" ago"_s);
        ...
        if (res->subscriptionMonths) {
          self->ui_.subageLabel->setText(QString("★ Subscribed for %2 months")
  ...
  case Qt::RightButton: {
    ...
    menu->addAction("Open channel in a new popup window", this, [username] {
      ...
      auto *split = app->getWindows()
                        ...
                        .getOrAddSelectedPage()
                        ->appendNewSplit(false);
      split->setChannel(app->getKickChatServer()->getOrCreate(username));
    });
    ...
    menu->addAction("Open channel in a new tab", this, [username] {
      ...
      split->setChannel(getApp()->getKickChatServer()->getOrCreate(username));
      container->insertSplit(split);
    });

    menu->addAction("Open channel in browser", this,
                    [channelURL] { QDesktopServices::openUrl(channelURL); });
...
void UserInfoPopup::appendCommonProfileActions(QMenu *menu) {
  if (!this->seventvUserID_.isEmpty()) {
    menu->addAction("Open 7TV user in browser", this,
...
// TimeoutWidget
//
UserInfoPopup::TimeoutWidget::TimeoutWidget() : BaseWidget(nullptr) {
  ...
  const auto addButton = [&](Action action, const QString &title,
    ...
    QObject::connect(button.getElement(), &Button::leftClicked, [this, action] {
